...
pub fn run_to_block(n: u64) {
    while System::block_number() < n {
        OcwDemo::on_finalize(System::block_number());
        System::on_finalize(System::block_number());
        System::set_block_number(System::block_number() + 1);
        System::on_initialize(System::block_number());
        OcwDemo::on_initialize(System::block_number());
    }
}

#[test]
fn submit_dotprice_signed_works() {
    let (mut t, _, _) = ExternalityBuilder::build();
    t.execute_with(|| {
        // call submit_dotprice_signed
        let dotprice = 18123;
        let acct: <TestRuntime as system::Trait>::AccountId = Default::default();
        assert_ok!(OcwDemo::submit_dotprice_signed(
            Origin::signed(acct),
            dotprice
        ));
        // A dotprice is inserted to <DotPrice> vec
        assert_eq!(<DotPrices>::get(), vec![dotprice]);
        // An event is emitted
        assert!(
            System::events()
                .iter()
                .any(|er| er.event
                    == TestEvent::ocw_demo(RawEvent::NewDotPrice(Some(acct), dotprice)))
        );

        // Insert another dotprice
        let dotprice2 = 18124;
        assert_ok!(OcwDemo::submit_dotprice_signed(
            Origin::signed(acct),
            dotprice2
        ...
        // A dotprice is inserted to <DotPrice> vec
        assert_eq!(<DotPrices>::get(), vec![dotprice, dotprice2]);
    });
}

#[test]
fn test_dotprice_signed_works() {
    let (mut t, _, _) = ExternalityBuilder::build();
    t.execute_with(|| {
        // call submit_dotprice_signed
        let dotprice = 18100;
        let acct: <TestRuntime as system::Trait>::AccountId = Default::default();
        let mut dotprices = VecDeque::new();
        ...
        for i in 0..100 {
            ...
            dotprices.push_back(dotprice + i);
            // A dotprice is inserted to <DotPrice> vec
            assert_eq!(<DotPrices>::get(), dotprices);
            // An event is emitted
            assert!(System::events().iter().any(|er| er.event
                == TestEvent::ocw_demo(RawEvent::NewDotPrice(Some(acct), dotprice + i))));
        }
    });
}
...
fn test_offchain_signed_tx() {
    ...
    t.execute_with(|| {
        // Setup
        let dotprice: u32 = 18123;
        OcwDemo::offchain_signed_tx(dotprice).unwrap();
        ...
        assert_eq!(tx.call, Call::submit_dotprice_signed(dotprice));
    });
}
...
fn test_offchain_unsigned_tx() {
    ...
    t.execute_with(|| {
        // when
        let dotprice: u32 = 18123;
        OcwDemo::offchain_unsigned_tx(dotprice).unwrap();
        // then
        let tx = pool_state.write().transactions.pop().unwrap();
        assert!(pool_state.read().transactions.is_empty());
        let tx = TestExtrinsic::decode(&mut &*tx).unwrap();
        assert_eq!(tx.signature, None);
        assert_eq!(tx.call, Call::submit_dotprice_unsigned(dotprice));
    });
}

#[test]
fn test_offchain_worker_works() {
    let (mut t, _, _) = ExternalityBuilder::build();
    t.execute_with(|| {
        run_to_block(10);

        assert_eq!(<DotPrices>::get().len(), 10);
    });
}
